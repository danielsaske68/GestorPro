...
class LiquidacionesFrame(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        # NO el directorio de trabajo actual. Así la carpeta de liquidaciones
        # siempre queda junto al programa, compilado o no.
        BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self.CARPETA_PDF = os.path.join(BASE_DIR, "liquidaciones_pdf")
    ...
    def generar_pdf(self):
        try:
            ...
            ruta_logo = r"C:\Users\Admin\Desktop\prueba\assets\logo.png"
            mes = self.mes_seleccionado.get().upper()
            filename = os.path.join(self.CARPETA_PDF, f"Liquidacion_{mes}.pdf")
            elements = []; styles = getSampleStyleSheet()
            if os.path.exists(ruta_logo):
                elements.append(RLImage(ruta_logo, width=120, height=120))
